    ...
    for (Edge<L> e : edges) {
      if (e.getSource().equals(source) && e.getTarget().equals(target)) {
        ...
        if (weight != 0) {
          ...
          edges.add(new Edge<L>(target, source, weight));
        }
        checkRep();
        return weight;
      }
...
/**
 * 图的边，由起点，终点，权重组成，其中起点和终点必须已经存在，权重必须为正整数 Immutable. This class is internal to
 * the rep of ConcreteEdgesGraph.
 * 
...
class Edge<L> {
   ...
   * 
   * @param source
   *          起点标签，非空，必须包含在vertices
   * @param target
   *          终点标签，非空，必须包含在vertices
   * @param weight
   *          权重，正整数
   */
  ...
  // TODO checkRep
  /**
   * 检查属性是否合法 1.source非空 2.target非空 3.权重为正整数
   */
  public void checkRep() {
    assert source != null;
    assert target != null;
