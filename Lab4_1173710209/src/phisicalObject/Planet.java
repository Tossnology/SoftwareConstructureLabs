...
public class Planet implements PhisicalObject {
	 ...
	 *   name:行星名字，类型为label
	 *   state:形态，只能为Solid或Liquid或Gas
	 *   color:颜色，类型为label
	 *   radius:行星半径，大于0
	 *   orbitRadius:轨道半径，大于0
	 *   speed:公转速度，大于0
	 *   direction:公转方向，只能为CW(顺)或CCW(逆)
	 *   initAngle:初始角度，[0,360)
	 ...
	 * @param speed 公转速度
	 * @param direction 公转方向
	 * @param initAngle 初始角度
	 */
	public Planet(String name,String state,String color,double radius,double orbitRadius,double speed,String direction,double initAngle) {
		this.name = name;
		this.state = state;
		this.color = color;
		this.radius = radius;
